...
size <- c(10, 20, 50, 100, 250, 500)

# probabilities for each pair of nodes to share an edge
prob_i <- c(2/9, 2/19, 2/49, 2/99, 2/249, 2/499)
prob_ii <- c(3/9, 3/19, 3/49, 3/99, 3/249, 3/499)
prob_iii <- c(4/9, 4/19, 4/49, 4/99, 4/249, 4/499)
prob_iv <- c(5/9, 5/19, 5/49, 5/99, 5/249, 5/499)
prob_v <- c(6/9, 6/19, 6/49, 6/99, 6/249, 6/499)

# names of graphs (add prob to graph name)
size_tokens <- c("s", "sm", "m", "l", "xl", "xxl")
...
for (i in c(1:1)){
  ...
  for (prob in probs){  
    ...
    weights_filename <- paste("data/DGPs/dag_", token,"_", prob ,"_weights.csv", sep="")
    write.csv(weights, weights_filename, row.names = FALSE)
    variance_filename <- paste("data/DGPs/dag_", token, "_", prob, "_error_var.csv", sep="")
    write.csv(error_var, variance_filename, row.names = FALSE)
  
    # retrieve Boolean adjacency matrix and store it
    amat <- as(graph, "matrix")
    amat_name <- paste("data/true_amat/dag_", token, "_", prob, ".csv", sep="")
    write.csv(amat, amat_name, row.names = FALSE)
  
    # create and store data
    data <- graph$simulate(n)
    filename <- paste("data/dag_", token, "_", prob, ".csv", sep="")
