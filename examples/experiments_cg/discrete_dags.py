...
# file for comparison
try:
    metadata = pd.read_csv("experiments_cg/results/discrete/metadata.csv")
except:
...
# dict for targets
targets_dict = {"alarm": "CATECHOL", "asia": "dysp", "hepar": "Cirrhosis", "sachs": "Erk"}
target = targets_dict[args.data]
...
# model prediction TODO: adapt to discrete data
def model_predict(x):
    return model.predict(x)


# TODO adapt to be compatible with explainer
def loss_fn(y, y_hat):
    return True


# set up sampler and decorrelator (same for Explainer and CGExplainer) # TODO simple sampler
...
ex_d_cg = wrk_cg.sage(X_test, y_test, partial_order, nr_orderings=orderings_sage.shape[0],
...
ex_d_cg_cd, orderings_cg_cd = wrk_cg.sage(X_test, y_test, partial_order, nr_runs=1)
time_cg_cd = time.time() - start_time_cg

# save  orderings
orderings_sage.to_csv(f'experiments_cg/results/discrete/orderings_sage_{args.data}_{args.model}.csv')
orderings_cg_cd.to_csv(f'experiments_cg/results/discrete/orderings_cg_cd_{args.data}_{args.model}.csv')
...
sage_values_ordering.to_csv(f"experiments_cg/results/discrete/sage_o_{args.data}_{args.model}.csv")
cg_values = ex_d_cg.scores.mean(level=0)
cg_values.to_csv(f"experiments_cg/results/discrete/cg_o_{args.data}_{args.model}.csv")
cg_cd_values = ex_d_cg_cd.scores.mean(level=0)
cg_cd_values.to_csv(f"experiments_cg/results/discrete/cg_cd_o_{args.data}_{args.model}.csv")

# fi_vals for the runs
ex_d_sage.fi_vals().to_csv(f"experiments_cg/results/discrete/sage_r_{args.data}_{args.model}.csv")
ex_d_cg.fi_vals().to_csv(f"experiments_cg/results/discrete/cg_r_{args.data}_{args.model}.csv")
ex_d_cg_cd.fi_vals().to_csv(f"experiments_cg/results/discrete/cg_cd_r_{args.data}_{args.model}.csv")


# fi_mean vals across runs + stds
ex_d_sage.fi_means_stds().to_csv(f"experiments_cg/results/discrete/sage_{args.data}_{args.model}.csv")
ex_d_cg.fi_means_stds().to_csv(f"experiments_cg/results/discrete/cg_{args.data}_{args.model}.csv")
ex_d_cg_cd.fi_vals().to_csv(f"experiments_cg/results/discrete/cg_cd_{args.data}_{args.model}.csv")
...
metadata.to_csv("experiments_cg/results/discrete/metadata.csv", index=False)
