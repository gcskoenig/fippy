...
class Decorrelator:
    ...
    def is_trained(self, K, J, C):
        ...
        trained = (K_key, J_key, C_key) in self._trained_sampling_funcs
        return trained
    ...
    def _store_decorrelationfunc(self, K : np.array, J : np.array, C : np.array, samplefunc : Callable[np.array, np.array], verbose=True):
        ...
        """
        K_key, C_key, J_key = Decorrelator._to_key(K), Decorrelator._to_key(C), Decorrelator._to_key(J)
        self._trained_sampling_funcs[(K_key, J_key, C_key)] = samplefunc
    ...
    def decorrelate(self, X_test : np.array, K : np.array, J : np.array, C : np.array, 
        ...
        # sample
        K_key, C_key, J_key = Decorrelator._to_key_(K), Decorrelator._to_key(C), Decorrelator._to_key(J)
        ...
        if not self.is_trained(K, J, C):
            ...
            decorrelated_data = decorrelationfunc(X_test, num_samples=num_samples)
